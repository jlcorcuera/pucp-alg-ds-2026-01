...
| [6. Hanoi Towers](#6-hanoi-towers) | Solve the classic Towers of Hanoi puzzle. |
| [7. Adjacent Hanoi Towers](#7-adjacent-hanoi-towers) | Solve the Towers of Hanoi puzzle with adjacent move restrictions. |
...
### 6. Hanoi Towers
Implement a recursive function to solve the classic Towers of Hanoi puzzle, moving $N$ disks from a source rod to a destination rod using an auxiliary rod.

* **Recursive Strategy:** To move $N$ disks from $S$ to $D$: 
    1. Move $N-1$ disks from $S$ to the auxiliary rod ($A$).
    2. Move the largest disk (disk $N$) from $S$ to $D$.
    3. Move the $N-1$ disks from $A$ to $D$.
    The base case occurs when $N=1$, where the single disk is moved directly.

**[Source Code](./hanoi_towers)**

### 7. Adjacent Hanoi Towers
Implement a recursive function to solve a variation of the Towers of Hanoi where disks can only be moved between adjacent rods (e.g., from Rod 1 to Rod 2, or Rod 2 to Rod 3, but never directly from Rod 1 to Rod 3).

* **Recursive Strategy:** To move $N$ disks from Rod 1 to Rod 3 under the adjacency constraint:
    1. Recursively move $N-1$ disks from Rod 1 to Rod 3.
    2. Move disk $N$ from Rod 1 to Rod 2.
    3. Recursively move $N-1$ disks from Rod 3 back to Rod 1.
    4. Move disk $N$ from Rod 2 to Rod 3.
    5. Recursively move $N-1$ disks from Rod 1 to Rod 3.
    This variation significantly increases the number of moves to $3^n - 1$.

**[Source Code](./adjacent_hanoi_towers)**
